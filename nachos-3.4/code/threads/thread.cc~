...
//----------------------------------------------------------------------
Thread* Thread::exist_Thread[128]={};
//exist_Thread[0]=currentThread;
Thread::Thread(char* threadName,int p=0)
{
    this->priority=p;
    bool judge=false;
    name = threadName;
    int temp=this->setThreadId();
    exist_Thread[temp]=this;
 // printf("%d\n",temp);
    if(temp==-1)  
	printf("超出线程数量限制，不能再创建线程!!!\n");
    else{
    threadptr[threadCount++]=this;
    Threads[temp]=1;
    this->threadId=temp;
 //   printf("%d\n",this->threadId=temp);
    stackTop = NULL;
