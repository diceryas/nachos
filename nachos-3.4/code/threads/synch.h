...
class Monitor_Pro{
	public:
		Monitor_Pro();
		~Monitor_Pro();
		void Produce_insert(Lock* conditionLock, int val);
		int Consumer_remove(Lock* conditionLock);
	private:
		int buffer[5];
		int count, hi, lo;
		Condition* condition;
...
class ReadWriteLock{
	public:
		...
		void WriteLock();
		void WriteUnlock();
		void setIsWriterFirst(bool WriterFirst){isWriterFirst=WriterFirst;}
		bool getIsWriterFirst(){return isWriterFirst;}
	private:
		char* name;
		bool isWriterFirst;
		Lock* mutex;
		Condition *readCondi, *writeCondi;
		int read_waiters;
		int write_waiters;
		int active_waiters;
		int active_readers;
