...
// from the virtual address
	...
	{
		if(phyState[i].vpn==vpn&&phyState[i].pageUsedThread==currentThread->getThreadId())
		{
			flag=true;
			break;
		}			
	}
	if(!flag)  return PageFaultException;
...
//region olivia lab4    
    if (writing){
        entry->dirty = TRUE;
        phyState[entry->physicalPage].dirty=true;
    }
	entry->lastUseTime=stats->totalTicks;
	//printf("use phyaddr %d\n",entry->physicalPage);
	phyState[entry->physicalPage].lastUseTime=stats->totalTicks;
//endregion  
